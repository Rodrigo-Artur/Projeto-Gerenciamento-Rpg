          ...
          return (
            <div key={participant.id} className={`rounded-xl border p-4 ${active ? "border-amber-500/60 bg-amber-500/10" : "border-zinc-800 bg-zinc-900/60"}`}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-[220px] flex-1"><div className="flex items-center gap-2"><input value={participant.name} onChange={(event) => patchParticipant(index, { name: event.target.value })} className="bg-transparent text-lg font-semibold text-zinc-100 outline-none" />{active && <span className="rounded-full bg-amber-500 px-2 py-0.5 text-xs font-semibold text-zinc-950">TURN0</span>}</div><div className="mt-2 flex flex-wrap gap-3 text-xs text-zinc-500"><label>Iniciativa <input type="number" value={participant.initiative} onChange={(event) => patchParticipant(index, { initiative: Number(event.target.value) })} className="ml-1 w-16 rounded border border-zinc-800 bg-zinc-950 px-2 py-1" /></label>{participant.armorClass !== undefined && <span className="flex items-center gap-1"><Shield className="h-3.5 w-3.5" />CA {participant.armorClass}</span>}<label className="flex items-center gap-1"><input type="checkbox" checked={Boolean(participant.hidden)} onChange={(event) => patchParticipant(index, { hidden: event.target.checked })} />Oculto no modo jogador</label></div></div>
...
function AbilityButton({ ability, onUse }: { ability: StructuredAbility; onUse: () => void }) {
  ...
  return <button onClick={onUse} disabled={unavailable} title={ability.effect} className="rounded-md border border-zinc-800 bg-zinc-900 p-2 text-left text-xs disabled:opacity-40"><div className="flex items-center justify-between gap-2"><span className="font-medium text-zinc-200">{ability.name}</span>{(ability.currentCooldown ?? 0) > 0 && <span className="text-amber-300">CD {ability.currentCooldown}</span>}</div><p className="mt-1 text-zinc-500">{ability.damage || ability.effect}</p>{ability.uses !== undefined && <p className="mt-1 text-zinc-600">Usos: {ability.uses}/{ability.maxUses ?? "—"}</p>}</button>;
}
