...
import type { WorkspaceTab } from "@/types/rulebook";
...
export function SystemWorkspace() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { setEditMode(false); setItemEditMode(false); }, delay);
  ...
  useEffect(() => {
    function handleShortcut(event: KeyboardEvent) {
      ...
      if ((event.ctrlKey || event.metaKey) && event.shiftKey && event.key.toLowerCase() === "p") { event.preventDefault(); openPlayerView(); }
      if ((event.ctrlKey || event.metaKey) && event.shiftKey && event.key.toLowerCase() === "c") { event.preventDefault(); toggleCompact(); }
      if ((event.ctrlKey || event.metaKey) && event.shiftKey && event.key.toLowerCase() === "e") { event.preventDefault(); setItemEditMode(false); setEditMode((current) => !current); setEditActivity(Date.now()); }
      if ((event.ctrlKey || event.metaKey) && !event.shiftKey && event.key.toLowerCase() === "z" && !insideEditableField) { event.preventDefault(); void runAction({ action: "undo-last", tableId: data.activeTableId }); }
    }
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      void search(searchQuery).then((results) => { setSearchResults(results); setSearchOpen(true); });
    }, 250);
    return () => window.clearTimeout(timer);
  }, [search, searchQuery]);
  ...
  function addTab(type: string, id: string, name: string) {
    ...
    setActiveTabId(tab.id); setItemEditMode(false);
  }

  function openContent(type: string, id: string, name: string) {
    const nextView = viewForType(type); setView(nextView); addTab(type, id, name); void recordRecent(type, id, name);
  }

  function openSearchResult(result: SearchResult) { openContent(result.type, result.id, result.name); setSearchQuery(""); setSearchOpen(false); }
  function openRecent(item: RecentContent) { openContent(item.type, item.id, item.name); }
  function opened(type: string, id: string, name: string) { addTab(type, id, name); void recordRecent(type, id, name); }

  function openTab(tab: WorkspaceTab) { setActiveTabId(tab.id); setView(tab.view as ViewId); setItemEditMode(false); void recordRecent(tab.type, tab.refId ?? tab.id, tab.title); }
  function closeTab(id: string) { setTabs((current) => current.filter((item) => item.id !== id)); if (activeTabId === id) { const remaining = tabs.filter((item) => item.id !== id); setActiveTabId(remaining.at(-1)?.id ?? ""); setItemEditMode(false); } }
  function detachTab(tab: WorkspaceTab) { window.open(`/focus?tableId=${encodeURIComponent(data.activeTableId)}&type=${encodeURIComponent(tab.type)}&id=${encodeURIComponent(tab.refId ?? "")}`, `mesa-focus-${tab.id.replaceAll(":", "-")}`, "width=700,height=900,resizable=yes,scrollbars=yes"); }

  async function exportCurrent() {
    const response = await fetch(`/api/rulebook?tableId=${encodeURIComponent(data.activeTableId)}&export=1`, { cache: "no-store" }); if (!response.ok) return;
    const json = await response.json(); const blob = new Blob([JSON.stringify(json, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `${(currentTable?.name ?? "mesa").replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-mesa-do-mestre-v4.json`; anchor.click(); URL.revokeObjectURL(url);
  }

  function openPlayerView() { window.open(`/player?tableId=${encodeURIComponent(data.activeTableId)}`, "mesa-do-mestre-player-view"); }
  function toggleCompact() { setCompact((current) => { const next = !current; window.localStorage.setItem("mesa-do-mestre-compact", next ? "1" : "0"); return next; }); }
  ...
  function canEditTarget(target: EventTarget | null) { if (editMode) return true; return itemEditMode && target instanceof HTMLElement && Boolean(target.closest(".item-editor-scope")); }
  function touchEdit() { if (effectiveEdit) setEditActivity(Date.now()); }
  function protectReadModeFormEvent(event: ReactFormEvent<HTMLElement>) { if (canEditTarget(event.target) || !protectedFieldFromTarget(event.target)) return; event.preventDefault(); event.stopPropagation(); }
  function protectReadModeClipboard(event: ReactClipboardEvent<HTMLElement>) { if (canEditTarget(event.target) || !protectedFieldFromTarget(event.target)) return; event.preventDefault(); event.stopPropagation(); }
  function protectReadModeKeyDown(event: ReactKeyboardEvent<HTMLElement>) { if (canEditTarget(event.target)) return; const field = protectedFieldFromTarget(event.target); if (!field) return; const key = event.key.toLowerCase(); const command = event.ctrlKey || event.metaKey; if (command && (key === "c" || key === "a")) return; if (key === "tab") return; if (!(field instanceof HTMLSelectElement) && ["arrowleft", "arrowright", "arrowup", "arrowdown", "home", "end", "pageup", "pagedown"].includes(key)) return; event.preventDefault(); event.stopPropagation(); }

  function handleDrop(event: React.DragEvent<HTMLElement>) {
    event.preventDefault(); const file = event.dataTransfer.files?.[0]; if (!file || !file.name.toLowerCase().endsWith(".json")) return;
    if (!editMode) { alert("Ative o Modo Edição antes de importar um arquivo."); return; }
    setDropFile(file); setImportOpen(true);
  }
  ...
  return <main onDragOver={(event) => event.preventDefault()} onDrop={handleDrop} className={`flex min-h-screen flex-col bg-zinc-950 text-zinc-100 ${compact ? "compact-workspace" : ""}`}>
    ...
    <section className={`workspace-content min-h-0 flex-1 overflow-hidden ${editMode ? "workspace-editing" : itemEditMode ? "workspace-readonly workspace-item-edit" : "workspace-readonly"}`} onBeforeInputCapture={protectReadModeFormEvent} onPasteCapture={protectReadModeClipboard} onCutCapture={protectReadModeClipboard} onKeyDownCapture={protectReadModeKeyDown} onInputCapture={touchEdit} onPointerDownCapture={touchEdit}>
      ...
      {view === "rules" && <div className={contentHeight}><RulesView data={data} action={runAction} onOpened={opened} /></div>}
      {view === "sheets" && <div className={contentHeight}><SheetsView data={data} action={runAction} onOpened={opened} /></div>}
      {view === "templates" && <div className={contentHeight}><TemplatesView data={data} action={runAction} /></div>}
      {view === "world" && <div className={contentHeight}><WorldView data={data} action={runAction} onOpened={opened} /></div>}
      {view === "campaign" && <div className={contentHeight}><CampaignToolsView data={data} action={runAction} /></div>}
      {view === "sessions" && <div className={contentHeight}><SessionsView data={data} action={runAction} onOpened={opened} /></div>}
